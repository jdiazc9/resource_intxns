...
for (si_plot in c(1, 9)) {
  ...
  ggplot(plot_this,
    ...
    ggtitle(paste(gsub('.RData', '', gsub('eval_data_', '', basename(file))),
                  '\n',
                  'training fraction = ', s[si_plot],
                  sep = ''))
  
  ggsave(filename = paste('../plots/', gsub('-', '', Sys.Date()), '-pred-vs-obs-s=', s[si_plot], '-', gsub('.RData', '', gsub('eval_data_', '', basename(file))), '.pdf', sep = ''),
         width = 220,
         height = 140,
         units = 'mm',
         limitsize = F)
...
ggplot(eval_stats_plot,
  ...
  scale_color_manual(name = 'Method',
                     values = c('#75a6eb',
                                ...
                                '#2b5c07',
                                '#c43833'),
                     labels = c('1st order linear regression - Lasso',
  ...
  ggtitle(gsub('.RData', '', gsub('eval_data_', '', basename(file))))

ggsave(filename = paste('../plots/', gsub('-', '', Sys.Date()), '-R2-vs-fraction-', gsub('.RData', '', gsub('eval_data_', '', basename(file))), '.pdf', sep = ''),
       width = 180,
       height = 140,
       units = 'mm',
       limitsize = F)
