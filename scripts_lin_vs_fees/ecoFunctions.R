...
inferAllResiduals <- function(ge_data) {
  ...
  M_sigma_pseudoinv <- tryCatch(
    MASS::ginv(M_sigma),
    error = function(e) {
      message("MASS::ginv failed, falling back to truncated SVD: ", e$message)
      sv <- svd(M_sigma, nu = 0, nv = 0)$d
      tol <- max(dim(M_sigma)) * .Machine$double.eps * max(sv)
      svd_r <- RSpectra::svds(M_sigma, k = sum(sv > tol))
      svd_r$v %*% diag(1 / svd_r$d) %*% t(svd_r$u)
    }
  )
